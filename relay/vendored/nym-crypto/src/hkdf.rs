...
// Tetherand local patch: digest 0.11 + hkdf 0.13 era.
//   - `digest::crypto_common` re-export removed → use `crypto_common`
//     crate directly.
//   - hkdf 0.13's `Hkdf<H>` is now a 1-generic type alias for
//     `GenericHkdf<Hmac<H>>` (was `Hkdf<D, H>`). For the SimpleHmac
//     variant the rename is `SimpleHkdf<D>`.
...
{
    // We discard the intermediate pseudorandom key from `extract`
    // because every call site here only needs the final `expand`
    // output (the OKM). If a future caller does need the PRK,
    // they should call `SimpleHkdf::<D>::extract(salt, ikm)` and
    // pass the result to `SimpleHkdf::<D>::from_prk(prk).expand(...)`
    // explicitly rather than going through this wrapper.
    //
    // hkdf 0.13: SimpleHkdf<D> = GenericHkdf<SimpleHmac<D>>
    let hkdf = SimpleHkdf::<D>::new(salt, ikm);
