...
./tutorial.sh --regen   # build HTML and exit</code><button onclick="copyCode(this)">copy</button></pre>
  ...
  <table>
    ...
    <tr><td><strong>M1</strong></td><td>Tether MVP: fork + rebrand, transport abstraction, USB-ADB + TCP transports, Compose Tether tab, <code>tetherand run</code> CLI. Replaces <code>connect.sh</code>.</td><td>10-14 h</td><td><span class="badge warn">NEXT</span></td></tr>
    <tr><td><strong>M2</strong></td><td>More transports: Bluetooth RFCOMM + USB-AOA, ratatui dashboard, LaunchAgent + IOKit USB watcher.</td><td>10-14 h</td><td>planned</td></tr>
    ...
    <tr><td><strong>M4</strong></td><td>Mullvad + PQ tunnel + multihop + DAITA + obfuscation toggles + kill-switch + split-tunnel.</td><td>8-12 h</td><td>planned</td></tr>
