...
./tutorial.sh --regen   # build HTML and exit</code><button onclick="copyCode(this)">copy</button></pre>
  ...
  <table>
    ...
    <tr><td><strong>M6</strong></td><td>Tor + Arti embedded: tetherand-tor Rust crate with arti-client 0.27 + BridgeDB-format bridge parser + vanguards toggle + PQ-NTor (prop362 / NTor-ML-KEM-v1) handshake preference + Privacy tab Tor config card + TorHop scaffolding + NDK cross-compile script. Pluggable transports (obfs4, snowflake, meek, webtunnel, conjure) + per-flow IP→arti forwarder + live bootstrap testing deferred to M6.x sub-plans.</td><td>~10 h shipped of 14-18 h</td><td><span class="badge ok">SCAFFOLDED</span></td></tr>
