...
./tutorial.sh --regen   # build HTML and exit</code><button onclick="copyCode(this)">copy</button></pre>
  ...
  <table>
    ...
    <tr><td><strong>M4e</strong></td><td>DAITA — Mullvad's Defense Against AI-guided Traffic Analysis via maybenot 2.2 framework.</td><td>~6 h</td><td><span class="badge ok">SHIPPED</span></td></tr>
    <tr><td><strong>M4f</strong></td><td>Obfuscation transports — UDP-over-TCP (length-prefixed), Shadowsocks (TCP+AEAD), QUIC (quinn over UDP/443).</td><td>~6 h</td><td><span class="badge ok">SHIPPED</span></td></tr>
    <tr><td><strong>M4g</strong></td><td>Split-tunnel by app — EncryptedSharedPreferences store + addDisallowedApplication in both VpnServices.</td><td>~3 h</td><td><span class="badge ok">SHIPPED</span></td></tr>
    <tr><td><strong>M5</strong></td><td>NymVPN mixnet hop: relay/nym Rust crate wrapping nym-sdk 1.4 (gated behind with-sdk feature; upstream nym-noise 1.20.4 has rustc 1.83+ inference bug); JNI surface; NymHop Sphinx-3-hop hop with mnemonic + entry/exit gateway config; NymCredentials EncryptedSharedPreferences persistence; Privacy tab Nym card; scripts/build-nym-android.sh.</td><td>~8 h</td><td><span class="badge ok">SHIPPED</span></td></tr>
    <tr><td><strong>M6</strong></td><td>Tor + Arti embedded: tetherand-tor Rust crate with arti-client 0.27 + BridgeDB-format bridge parser + vanguards toggle + PQ-NTor (prop362 / NTor-ML-KEM-v1) handshake preference + Privacy tab Tor config card + TorHop + NDK cross-compile script.</td><td>~10 h</td><td><span class="badge ok">SHIPPED</span></td></tr>
    <tr><td><strong>M6.x</strong></td><td>PT bridges + per-flow forwarder + live probe: tetherand-pt Rust binary implementing obfs4 + meek + webtunnel inline (5/5 tests); arti managed-PT wiring via TransportConfigBuilder; TorFlowForwarder per-flow IP→arti DataStream forwarder; PtBinaryStager extracting bundled PTs from nativeLibraryDir; cross-compile scripts for snowflake-client + conjure-client (Go upstream); live-probe integration tests for clearnet + .onion.</td><td>~14 h</td><td><span class="badge ok">SHIPPED</span></td></tr>
    <tr><td><strong>M7a</strong></td><td>Threat Detection MVP — NetMonster Tier 0 + AIMSICD BTSAlgorithm + SnoopSnitch RAT-downgrade + Crocodile Hunter (TAC / EARFCN / re-attach-storm) + Wi-Fi evil-twin + BLE tracker fingerprints + app-audit + per-geohash6 baseline + Threat tab + panic button. OpenCellID bundle replaced by per-location baseline detection.</td><td>~20 h</td><td><span class="badge ok">SHIPPED</span></td></tr>
    ...
    <tr><td><strong>M8</strong></td><td>Polish &amp; release: <code>make native-all</code> orchestrates all 5 native-lib cross-compiles; <code>make release-signed</code> via scripts/release-sign.sh (PII-DN-allow-list gate + apksigner SHA256-RSA2048 v2 scheme); <code>make smoke-device</code> walks every tab via UiAutomator + dumpsys; <code>make hashes</code> + scripts/hash-artifacts.sh emit SHA-256 + SHA3-256 sidecars + SHASUMS.txt for every bin/ artefact; <code>make launcher</code> installs the macOS LaunchAgent.</td><td>~6 h</td><td><span class="badge ok">SHIPPED</span></td></tr>
