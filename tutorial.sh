...
./tutorial.sh --regen   # build HTML and exit</code><button onclick="copyCode(this)">copy</button></pre>
  ...
  <table>
    ...
    <tr><td><strong>M10</strong></td><td>AI-era defenses (local-only, contributory): deterministic primaries first, then 4-model classifier stack on NPU, prompt-injection clipboard scrubber, C2PA / SynthID provenance, OSINT exposure dashboard, NPU sysfs watcher, egress-LLM-API SNI watch.</td><td>26-34 h</td><td><span class="badge warn">NEXT</span></td></tr>
  </table>
